 ...
 *
 * @technique
 * Uses custom generalization of barycentric coordinates.
 * RESULT: failed, this doesnt work
 */
...
const screen = document.getElementById("screen-bary");
screen.width = WIDTH;
...
/**
 * Calculates & returns barycentric coordinates of a point locates within the canvas square.
 ...
 * @return {object<number, 4>} barycentric coordinates/weights
 *
 * @pathflow
 * For any point P inside square ABCD, we get 4 triangles so formed.
 * Using these we can determine 4 weights corresponding to each vertex and adding up to 1.
 * Say, for vertex A, we take triangles **EXCLUDING POINT A**, i.e. ∆PBC & ∆PCD.
 * Then we compute the areas of the 2 offset triangles and divide it by square's total area.
 * We proceed cyclically across rest all vertices & get all weights.
 * However, right now, adding up all will yield sum 2.
 * So, we divide all by 2 to normalize.
 *
 * @key_insight:
 * This can be easily extended for any point P inside any polygon with k number of vertices.              
 * For such, we would consider the (k-2) triangles out of the total k triangles so formed.
 * Then divide each by polygon's total area.
 * Since this would add up to (k-2), we would finally divide by (k-2) to normalize.
 *
 * @optimization
...
function getBarycentricCoords(p) {
  // << get offset areas >> \\
  const area0 = getAreaByCoords(p, v1, v2) + getAreaByCoords(p, v2, v3);
  const area1 = getAreaByCoords(p, v2, v3) + getAreaByCoords(p, v3, v0);
  const area2 = getAreaByCoords(p, v3, v0) + getAreaByCoords(p, v0, v1);
  const area3 = getAreaByCoords(p, v0, v1) + getAreaByCoords(p, v1, v2);
  // << weights >> \\
  return {
    w0: (0.5 * area0) / TOTAL_AREA,
    w1: (0.5 * area1) / TOTAL_AREA,
    w2: (0.5 * area2) / TOTAL_AREA,
    w3: (0.5 * area3) / TOTAL_AREA
  }
}
 ...
 * @param {object<number, 4>} weights, barycentric coordinates/weights
 ...
 */
let timeRecords = []; // holds execution timestamps
function paintScreen() {
  // << setup >> \\
  ...
  for (let x=0; x<WIDTH; x++) {
    for (let y=0; y<HEIGHT; y++) {
      // << interpolation math >> \\
      const rawColour = getInterpolatedColour(getBarycentricCoords({x: x, y: y}));
      // << paint >> \\
